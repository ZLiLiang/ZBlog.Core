...
namespace Ocelot.Provider.Nacos.NacosClient.LoadBalance
{
    public class WeightRandomLBStrategy : ILBStrategy
    {
        ...
        private Dictionary<string, double> BuildScore(List<Instance> list)
        {
            ...
            list.ForEach(x =>
            {
                x.InstanceId = string.IsNullOrWhiteSpace(x.InstanceId) ?
                    $"{x.Ip}#{x.Port}#{x.ClusterName}#{x.ServiceName}" :
                    x.InstanceId;
            });

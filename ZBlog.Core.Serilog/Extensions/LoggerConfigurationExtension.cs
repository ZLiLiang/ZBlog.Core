...
namespace ZBlog.Core.Serilog.Extensions
{
    public static class LoggerConfigurationExtension
    {
        public static LoggerConfiguration WriteToConsole(this LoggerConfiguration loggerConfiguration)
        {
            ...
            loggerConfiguration = loggerConfiguration.WriteTo
                .Logger(lg => lg.FilterRemoveSqlLog().WriteTo.Console());

            //输出SQL
            loggerConfiguration = loggerConfiguration.WriteTo
                .Logger(lg => lg.FilterSqlLog().Filter.ByIncludingOnly(Matching.WithProperty<bool>(LogContextStatic.SqlOutToConsole, s => s)))
                .WriteTo
                .Console();
        ...
        public static LoggerConfiguration WriteToFile(this LoggerConfiguration loggerConfiguration)
        {
            //输出SQL
            loggerConfiguration = loggerConfiguration.WriteTo
                .Logger(lg => lg.FilterSqlLog().Filter.ByIncludingOnly(Matching.WithProperty<bool>(LogContextStatic.SqlOutToFile, s => s))
            ...
            //输出普通日志
            loggerConfiguration = loggerConfiguration.WriteTo
                .Logger(lg => lg.FilterRemoveSqlLog().WriteTo.Async(s => s.File(LogContextStatic.Combine(LogContextStatic.BasePathLogs, @"Log.txt"),
                                                                               rollingInterval: RollingInterval.Day,
                                                                               outputTemplate: LogContextStatic.FileMessageTemplate, retainedFileCountLimit: 31)));
            return loggerConfiguration;
        }
        ...
        public static IEnumerable<LogEvent> FilterSqlLog(this IEnumerable<LogEvent> batch)
        {
            ...
            return batch.Where(s => s.WithProperty<string>(LogContextStatic.LogSource,
                                                           q => LogContextStatic.AopSql.Equals(q)))
        ...
        public static LoggerConfiguration FilterRemoveSqlLog(this LoggerConfiguration loggerConfiguration)
        {
        ...
        {
            return batch.Where(s => WithProperty<string>(LogContextStatic.LogSource, q => !LogContextStatic.AopSql.Equals(q))(s));
        }
        ...
        public static bool WithProperty<T>(this LogEvent logEvent, string key, Func<T, bool> predicate)
        {

...
using ZBlog.Core.Model.Logs;
using ZBlog.Core.Serilog.Extensions;

namespace ZBlog.Core.Serilog.Sink
{
    public class LogBatchingSink : IBatchedLogEventSink
    {
        ...
        private async Task WriteLogs(ISqlSugarClient db, IEnumerable<LogEvent> batch)
        {
            ...
            var group = batch.GroupBy(it => it.Level);
            foreach (var item in group)
            {
                switch (item.Key)
                {
                    case LogEventLevel.Information:
                        await WriteInformationLog(db, item);
                        break;
                    case LogEventLevel.Warning:
                        await WriteWarningLog(db, item);
                        break;
                    case LogEventLevel.Error:
                    case LogEventLevel.Fatal:
                        await WriteErrorLog(db, item);
                        break;
                }
            }
        }

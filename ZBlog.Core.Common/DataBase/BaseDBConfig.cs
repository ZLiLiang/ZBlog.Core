...
namespace ZBlog.Core.Common.DataBase
{
    public class BaseDBConfig
    {
        ...
        {
            List<MutiDBOperate> mutilDB = AppSettings.App<MutiDBOperate>("DBS")
                .Where(i => i.Enabled)
                .ToList();
            var mainDBId = AppSettings.App(new string[] { "MainDB" }).ObjToString();
            var mainDBModel = mutilDB.Single(d => d.ConnId == mainDBId);
            mutilDB.Remove(mainDBModel);
            mutilDB.Insert(0, mainDBModel);

            foreach (var item in mutilDB)
                SpecialDbString(item);

            return (mutilDB, mainDBModel.Slaves);
        }

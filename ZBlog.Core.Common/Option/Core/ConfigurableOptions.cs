...
namespace ZBlog.Core.Common.Option.Core
{
    public static class ConfigurableOptions
    {
        ...
        public static IServiceCollection AddConfigurableOptions(this IServiceCollection services, Type type)
        {
            ...
            services.AddOptions();
            services.AddSingleton(iOptionsChangeTokenSource, Activator.CreateInstance(configurationChangeTokenSource, Options.DefaultName, config) ?? throw new InvalidOperationException());
            services.AddSingleton(iConfigureOptions, Activator.CreateInstance(namedConfigureFromConfigurationOptions, Options.DefaultName, config) ?? throw new InvalidOperationException());
        ...
        public static string GetConfigurationPath(Type optionsType)
        {
            ...
            {
                if (configurationPath.EndsWith(item))
                {
                    return configurationPath[..^item.Length];
                }
            }

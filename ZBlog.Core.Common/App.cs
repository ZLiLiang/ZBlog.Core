...
namespace ZBlog.Core.Common
{
    public class App
    {
        ...
        public static IServiceProvider GetServiceProvider(Type serviceType, bool mustBuild = false, bool throwException = true)
        {
            if (HostEnvironment == null || RootServices != null &&
                InternalApp.InternalServices
                    .Where(s => s.ServiceType == (serviceType.IsGenericType ? serviceType.GetGenericTypeDefinition() : serviceType))
                    .Any(s => s.Lifetime == ServiceLifetime.Singleton))
                return RootServices;
        ...
        {
            return GetService(typeof(TService), null, mustBuild) as TService;
        }
        ...
        {
            return (serviceProvider ?? GetServiceProvider(typeof(TService), mustBuild, false))?.GetService<TService>();
        }
        ...
        {
            return (serviceProvider ?? GetServiceProvider(type, mustBuild, false))?.GetService(type);
        }
        ...
        private static IEnumerable<Type> GetTypes(Assembly assembly)
        {
            ...
            {
                $@"Error load `{assembly.FullName}` assembly.".WriteErrorLine();
            }
            return source.Where(t => t.IsPublic);
        }

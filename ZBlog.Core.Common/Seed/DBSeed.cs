...
namespace ZBlog.Core.Common.Seed
{
    public class DBSeed
    {
        ...
        {
            // 获取所有种子配置-初始化数据
            var seedDataTypes = AssemblysExtension.GetAllAssemblies()
        ...
        public static void MigrationLogs(MyContext myContext)
        {
            ...
            modelTypes.ForEach(it =>
            {
                // 这里只支持添加修改表，不支持删除
                // 如果想要删除，数据库直接右键删除，或者联系SqlSugar作者；
                if (!tables.Any(s => s.Name.Contains(it.Name)))
                {
                    Console.WriteLine(it.Name);
                    if (it.GetCustomAttribute<SplitTableAttribute>() != null)
                    {
                        logDb.CodeFirst.SplitTables().InitTables(it);
                    }
                    else
                    {
                        logDb.CodeFirst.InitTables(it);
                    }
                }
            });
            ...
            $"Log Tables created successfully! {stopwatch.ElapsedMilliseconds}ms".WriteSuccessLine();
            Console.WriteLine();
        }
        ...
        {
            var tenants = await myContext.Db.Queryable<SysTenant>()
            ...
            tenants = await myContext.Db.Queryable<SysTenant>()
                .Where(s => s.TenantType == TenantTypeEnum.Tables)
                .ToListAsync();
            if (tenants.Any())
            {
        ...
        private static async Task TenantSeedDataAsync(ISqlSugarClient db, TenantTypeEnum tenantType)
        {
            // 获取所有种子配置-初始化数据
            var seedDataTypes = AssemblysExtension.GetAllAssemblies()
                .SelectMany(it => it.DefinedTypes)
                .Where(it => !it.IsInterface && !it.IsAbstract && it.IsClass)
                .Where(it =>
                {
                    var esd = it.GetInterfaces()
                        .FirstOrDefault(it => it.HasImplementedRawGeneric(typeof(IEntitySeedData<>)));
                    if (esd is null)
                        return false;

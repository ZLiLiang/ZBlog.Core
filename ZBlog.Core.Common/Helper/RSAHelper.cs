            ...
            using (MemoryStream msInput = new MemoryStream(Convert.FromBase64String(cipherText)))
            {
                using (MemoryStream msOutput = new MemoryStream())
                {
                    ...
                    while ((readLen = msInput.Read(buffer, 0, bufferSize)) > 0)
                    {
                        ...
                        byte[] decData = _privateKeyRsaProvider.Decrypt(dataToDec, RSAEncryptionPadding.Pkcs1);
            ...
            using (MemoryStream msInput = new MemoryStream(_encoding.GetBytes(text)))
            {
                using (MemoryStream msOutput = new MemoryStream())
                {
                    ...
                    while ((readLen = msInput.Read(buffer, 0, bufferSize)) > 0)
                    {
                        ...
                        byte[] encData = _publicKeyRsaProvider.Encrypt(dataToEnc, RSAEncryptionPadding.Pkcs1);

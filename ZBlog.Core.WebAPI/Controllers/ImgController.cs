...
namespace ZBlog.Core.WebAPI.Controllers
{
    ...
    public class ImgController : BaseApiController
    {
        ...
        {
            string foldername = "";
            string filepath = Path.Combine(_environment.WebRootPath, foldername, "测试下载中文名称的图片.png");
        ...
        public async Task<MessageModel<string>> InsertPicture([FromForm] UploadFileDto dto)
        {
            ...
            foreach (var file in allowedFile)
            {
                ...
                var path = Path.Combine(_environment.WebRootPath, strpath);
        ...
        public FileStreamResult DownBmd(string filename)
        {
            ...
            string filepath = Path.Combine(_environment.WebRootPath, Path.GetFileName(filename));

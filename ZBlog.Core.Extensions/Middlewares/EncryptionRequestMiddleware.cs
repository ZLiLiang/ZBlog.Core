...
namespace ZBlog.Core.Extensions.Middlewares
{
    public class EncryptionRequestMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (AppSettings.App("Middleware", "EncryptionRequest", "Enabled").ObjToBool())
            {
                ...
                if (isAllApis || (path.Length > 5 && needEnApis.Any(it => it.ToLower().Contains(path))))
                {
    ...
    public static class EncryptionRequestExtension
    {
        /// <summary>
        /// 自定义中间件
        /// 通过配置，对指定接口入参进行解密操作
        ...
        /// <returns></returns>
        public static IApplicationBuilder UseEncryptionRequest(this IApplicationBuilder app)
        {
            return app.UseMiddleware<EncryptionRequestMiddleware>();
        }
    }
}

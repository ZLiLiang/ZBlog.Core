...
using ZBlog.Core.Extensions.NacosConfig;
    ...
    public static class NacosSetup
    {
        public static void AddNacosSetup(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            if (AppSettings.App(new string[] { "Startup", "Nacos", "Enabled" }).ObjToBool())
            {
                ...
                {
                    config.ServerAddresses = JsonConfigSettings.NacosServerAddresses;
                    config.EndPoint = "";
                    config.Namespace = JsonConfigSettings.NacosNamespace;
                    config.DefaultTimeOut = JsonConfigSettings.NacosDefaultTimeOut;
                    config.ListenInterval = JsonConfigSettings.ListenInterval;
                    // swich to use http or rpc
                    config.ConfigUseRpc = false;
                });

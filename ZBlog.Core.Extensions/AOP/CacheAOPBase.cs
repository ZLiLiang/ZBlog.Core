...
namespace ZBlog.Core.Extensions.AOP
{
    public abstract class CacheAOPBase : IInterceptor
    {
        ...
        protected string CustomCacheKey(IInvocation invocation)
        {
            ...
            var methodArguments = invocation.Arguments.Select(GetArgumentValue)
                .Take(3)
                .ToList();//获取参数列表，最多三个
        ...
        private static string Resolve(Expression expression)
        {
            ...
            expressionContext.Resolve(expression, ResolveExpressType.WhereSingle);
            var value = expressionContext.Result.GetString();
            var parameters = expressionContext.Parameters;

            parameters.ForEach(it =>
            {
                value = value.Replace(it.ParameterName, it.Value.ObjToString());
            });

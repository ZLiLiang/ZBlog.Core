...
namespace ZBlog.Core.Extensions.AOP
{
    ...
    public class CacheAOP : CacheAOPBase
    {
        ...
        public override void Intercept(IInvocation invocation)
        {
            ...
            //对当前方法的特性验证
            //如果需要验证
            var cachingAttribute = method.GetCustomAttributes(true)
                .FirstOrDefault(it => it.GetType() == typeof(CachingAttribute));
            if (cachingAttribute is CachingAttribute cachingAttr)
            {
                //获取自定义缓存键
                ...
                if (!string.IsNullOrWhiteSpace(cacheKey))
                {
                    ...
                    _cache.Set(cacheKey, response, TimeSpan.FromMinutes(cachingAttr.AbsoluteExpiration));
                }
            }
            else
            {

...
namespace ZBlog.Core.Repository.Base
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, new()
    {
        ...
        private ISqlSugarClient _db
        {
            get
            {
                ...
                if (mta is { TenantType: TenantTypeEnum.Db })
                {
                    ...
                    {
                        var tenant = db.Queryable<SysTenant>()
                            .WithCache()
                            .Where(s => s.Id == App.User.TenantId)
                            .First();
                        if (tenant != null)
                        {
        ...
        public async Task<long> Add(TEntity entity, Expression<Func<TEntity, object>> insertColumns = null)
        {
            ...
            if (insertColumns == null)
                ...
                return await insert.InsertColumns(insertColumns)
        ...
        {
            return await _db.Deleteable(model)
                 .ExecuteCommandHasChangeAsync();
        }
        ...
        {
            return await _db.Updateable<TEntity>(operateAnonymousObjects)
        ...
        public async Task<bool> Update(
            ...
            string where = "")
        {
            IUpdateable<TEntity> updateable = _db.Updateable(entity);
            if (lstIgnoreColumns != null && lstIgnoreColumns.Count > 0)
                updateable = updateable.IgnoreColumns(lstIgnoreColumns.ToArray());
            if (lstColumns != null && lstColumns.Count > 0)
                updateable = updateable.UpdateColumns(lstColumns.ToArray());
        ...
        /// <param name="whereExpression">查询实体条件</param>
        ...
        public async Task<List<TEntity>> Query(Expression<Func<TEntity, bool>> whereExpression, string orderByFields)
        {
            return await _db.Queryable<TEntity>()
                .WhereIF(whereExpression != null, whereExpression)
                .OrderByIF(orderByFields != null, orderByFields)
                .ToListAsync();
        }
            ...
            string orderByFields = null)
        {
            ...
            return new PageModel<TEntity>(pageIndex, totalCount, pageSize, result);
        }
        ...
        {
            RefAsync<int> totalCount = 0;
            var result = await _db.Queryable<T, T2>(joinExpression)
                .Select(selectExpression)
                .OrderByIF(!string.IsNullOrEmpty(orderByFields), orderByFields)
                .WhereIF(whereExpression != null, whereExpression)
                .ToPageListAsync(pageIndex, pageSize, totalCount);

            return new PageModel<TResult>(pageIndex, totalCount, pageSize, result);
        }
        ...
        {
            //精准找单个表
            var tableName = _db.SplitHelper<TEntity>()
                .GetTableName(dateTime);//根据时间获取表名
